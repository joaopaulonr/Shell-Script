...
aws ec2 authorize-security-group-ingress --group-id ${SECGRUPID} --protocol tcp --port 5432 --source-group ${SECGRUPID} > /dev/null
#Criação-da-instância.
aws ec2 run-instances --image-id ${ISO} --instance-type t2.micro --key-name ${KEY} --subnet-id ${SUBNET} --security-group-ids ${SECGRUPID} --user-data file://mySql.sh >> aux.txt
